-- Verification read-only for Basic profile cosmetics assets added on 2026-08-24.
...
found as (
  ...
  from proposed
  left join public.portal_cosmetic_assets asset
    on asset.asset_key = proposed.asset_key
  left join public.portal_cosmetic_collections collection
    on collection.id = asset.collection_id
)
select
  count(*) filter (where id is not null) as found_assets,
  count(*) filter (where id is null) as missing_assets,
  count(*) filter (where collection_key = 'basic') as in_basic_collection,
  count(*) filter (where is_active = true) as active_assets,
  count(*) filter (where asset_type = 'avatar') as avatars_found,
  count(*) filter (where asset_type = 'frame') as frames_found
from found;
...
select
  proposed.asset_key,
  asset.id is not null as exists_in_catalog,
  asset.display_name = proposed.display_name as display_name_ok,
  asset.asset_type = proposed.asset_type as asset_type_ok,
  asset.asset_url = proposed.asset_url as asset_url_ok,
  asset.sort_order = proposed.sort_order as sort_order_ok,
  asset.metadata = proposed.metadata as metadata_ok,
  collection.collection_key = 'basic' as collection_ok,
  asset.is_active = true as active_ok
from proposed
left join public.portal_cosmetic_assets asset
  on asset.asset_key = proposed.asset_key
left join public.portal_cosmetic_collections collection
