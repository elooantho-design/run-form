...
try {
  ...
  console.error("Réponse non JSON api/run-add:", rawText);
  setSaveMessage(`Erreur création run : réponse non JSON (${response.status})`);
  return;
}

if (!response.ok) {
  console.error("Erreur api/run-add:", data);
  setSaveMessage(
    `Erreur création run : ${data?.error || "erreur inconnue"}`
  );
  return;
}

    setSaveMessage(`Run #${data.strat_id} enregistré avec succès.`);
    resetAll();
  } catch (error) {
      ...
      <Card className="rounded-3xl border-zinc-800 bg-zinc-900/70 shadow-2xl">
        <CardHeader className="border-b border-zinc-800">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <CardTitle className="text-lg text-zinc-100">{t("run.add.title", "Ajout de run")}</CardTitle>
...
<Button
  ...
  onClick={saveRun}
>
  {saveLoading ? t("run.saving", "Enregistrement...") : t("common.save", "Enregistrer")}
</Button>
{saveMessage ? (
            ...
            <Card className="rounded-3xl border-zinc-800 bg-zinc-950/60">
              ...
              <CardContent className="space-y-4 p-4">
                ...
                <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 text-sm text-zinc-400">
                  {t("run.scopeNoticeAdd", "Le run sera enregistre dans la banque de l'espace connecte.")}
                </div>
              </CardContent>
            </Card>
          </div>
